...
class TestWorkflow(unittest.TestCase):
    ...
    def test_unconstrained_reconciliation(self):
        ...
        accuracy_base = ht.accuracy_base(
            test, base_forecasts, hist=train, measure=['mase', 'rmse'])
    ...
    def test_constrained_reconciliation(self):
        ...
        self.assertTrue(np.allclose(
            base_forecasts[:, [0]], ht.aggregate_ts(ols1, ht.node_name[0])))
        ...
        ols1 = model_ols.predict(horizon=12)
        self.assertTrue(np.allclose(
            base_forecasts[:, [0, 100]], ht.aggregate_ts(ols1, ht.node_name[[0, 100]])))
...
class TestTemporalWorkflow(unittest.TestCase):
    ...
    def test_aggregate_ts(self):
        ...
        self.assertTrue(np.allclose(
            ats['agg_2'], nonoverlapping_aggregate(bts, 2)))
        self.assertTrue(np.allclose(
            ats['agg_3'], nonoverlapping_aggregate(bts, 3)))
        self.assertTrue(np.allclose(
            ats['agg_4'], nonoverlapping_aggregate(bts, 4)))
        self.assertTrue(np.allclose(
            ats['agg_6'], nonoverlapping_aggregate(bts, 6)))
        self.assertTrue(np.allclose(
            ats['agg_12'], nonoverlapping_aggregate(bts, 12)))
    ...
    def test_dict2array(self):
        ...
        for i in range(8):
            self.assertTrue(np.allclose(
                tmp2[i, 1:3], tmp1['agg_6'][i*2:2*(i+1)]))
    ...
    def test_workflow(self):
        ...
        accs = [ht.accuracy(np.random.random(24), f, ts)
                for f in (ols, wlss, wlsv, shrink)]
